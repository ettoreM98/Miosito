...
<button onclick="login()">ENTRA NEL GIOCO</button>    
</div>    
...
Verrà sotto posta a un quiz su delle semplicissime domande di cultura generale.<br>    
...
Se ha capito al citazione può proseguire col quiz cliccando sul tasto start altrimenti può’ chiudere tutto qui e andare a casa.<br><br>    
...
function login(){    
if(user.value==="regalotardi"&&pass.value==="shein"){    
...
quiz.innerHTML=`<div class="timer" id="timer">${timer.innerText}</div><h2>${current+1}) ${q.q}</h2>`;    
q.a.forEach((ans,i)=>{    
...
function showResult(){    
if(score>=6){    
result.innerHTML=`<h2>ACCESSO CONSENTITO</h2>    
<img src="https://i.imgur.com/8RkR5QK.png">    
<p>iPad A16 rosa</p>`;    
}else{    
result.innerHTML=`<h2>ELIMINATO</h2>    
